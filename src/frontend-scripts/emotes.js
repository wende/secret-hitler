...
export function processEmotes(input, isMod, allEmotes) {
	...
	message.forEach((word, index) => {
		const validSiteURL = /^http[s]?:\/\/(secrethitler\.io|localhost:8080|github\.com\/cozuya\/secret-Wende)\/([a-zA-Z0-9#?=&\/\._]*)$/i;
		if (mapping[word]) {
			formatedMsg.push(
			...
			const isGithub = data[1] == 'github.com/cozuya/secret-Wende';
			const gameURL = data[2].startsWith('game/');
			/* eslint-disable */
			formatedMsg.push(
				<a
					key={index}
					href={isGithub ? 'https://github.com/cozuya/secret-Wende/' + data[2] : gameURL ? '/game/' + data[2].substring(5) : '/' + data[2]}
					className="shio-link"
